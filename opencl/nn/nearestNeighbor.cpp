...
float init_time = 0, mem_alloc_time = 0, h2d_time = 0, exec_time = 0,
      d2h_time = 0, close_time = 0, total_time = 0;
size_t mem_alloc_bytes = 0, h2d_bytes = 0, d2h_bytes = 0;
#endif
char *data_folder;

cl_context context = NULL;
int platform = -1, device = -1;
...
float *OpenClFindNearestNeighbors(
	...
	mem_alloc_bytes += sizeof(LatLong) * numRecords;
	mem_alloc_bytes += sizeof(float) * numRecords;
#endif
...
#ifdef TIMING
	h2d_bytes += sizeof(LatLong) * numRecords;
#endif
	...
	error = clEnqueueReadBuffer(command_queue,
			...
			&readEvent);
#ifdef TIMING
	d2h_bytes += sizeof(float) * numRecords;
#endif
	...
	if (timing) {
		...
		totalTime += eventEnd-eventStart;
		exec_time = (eventEnd - eventStart) / 1e6;
	...
	printf("\nkernel_name,mem_alloc_bytes,mem_alloc_time,HtoD_bytes,HtoD_time,exec_time,DtoH_bytes,DtoH_time,total_time,platform,device\n");
        printf("%s,%zd,%f,%zd,%f,%f,%zd,%f,%f,%d,%d\n", "NearestNeighbor",
                mem_alloc_bytes, mem_alloc_time,
                h2d_bytes, h2d_time,
                exec_time,
                d2h_bytes, d2h_time,
                mem_alloc_time + h2d_time + exec_time + d2h_time,
		platform, device);
#endif
	return distances;
}
