...
extern float init_time, mem_alloc_time, h2d_time,
       d2h_time, close_time, total_time;

#include <vector>
extern vector<float> exec_time;

extern size_t mem_alloc_bytes, h2d_bytes, d2h_bytes;
#endif
...
OpenCL::~OpenCL()
{
	...
	for (int i = 0; i < exec_time.size(); ++i) {
		printf("%s_%d,%zd,%f,%zd,%f,%f,%zd,%f,%f,%d,%d\n",
			"dynproc_kernel", i,
			mem_alloc_bytes, mem_alloc_time,
			h2d_bytes, h2d_time,
			exec_time[i],
			d2h_bytes, d2h_time,
			mem_alloc_time + h2d_time + exec_time[i] + d2h_time,
...
void OpenCL::launch(string toLaunch)
{
	...
	exec_time.push_back(probe_event_time(event, command_queue));
}
