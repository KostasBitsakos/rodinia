...
int main(int argc, char** argv)
{
	...
	printf("Sorting list of %lld floats.\n", numElements);
	...
	for (kern_t k = 0; k < kern_NUM_KERNELS; ++k) {
		fprintf(stderr, "%s,%zd,%f,%zd,%f,%f,%zd,%f,%f,%d,%d\n", kern_name[k],
			mem_alloc_bytes[k], mem_alloc_time[k],
			h2d_bytes[k], h2d_time[k],
			exec_time[k],
			d2h_bytes[k], d2h_time[k],
			mem_alloc_time[k] + h2d_time[k] + exec_time[k] + d2h_time[k],
